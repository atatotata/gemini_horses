#!/usr/bin/env python3
"""Debug storyrace MonoBehaviour typetree."""
...
for obj in env.objects:
    if obj.type.name == "MonoBehaviour":
        try:
            ...
            if tree:
                print(f"Typetree type: {type(tree)}")
                                ...
                                if isinstance(item, dict):
                                    ...
                                    for ik, iv in item.items():
                                        if isinstance(iv, str):
                                            print(f"      {ik}: {repr(iv)[:300]}")
                                        elif isinstance(iv, list):
                                            print(f"      {ik}: list[{len(iv)}]")
                                            for j, sub in enumerate(iv[:3]):
                                                print(f"        [{j}]: {repr(sub)[:200]}")
                                        else:
                                            print(f"      {ik}: {type(iv).__name__}={iv}")
                                elif isinstance(item, str):
                                    print(f"    [{i}]: {repr(item)[:200]}")
                                else:
                                    print(f"    [{i}]: {type(item).__name__}={repr(item)[:200]}")
...
# Also check a sample existing storyrace for reference
print("\n\n=== EXISTING STORYRACE storyrace_020000001 ===")
existing = json.load(open(r"G:\Games\steamapps\common\UmamusumePrettyDerby_Jpn\gemini_horses\localized_data\assets\race\storyrace\text\storyrace_020000001.json", 'r', encoding='utf-8'))
print(f"Type: {type(existing)}, len: {len(existing)}")
