#!/usr/bin/env python3
"""Full extraction pipeline: write typetree to file for storyrace_020290011."""
...
for obj in env.objects:
    if obj.type.name == "MonoBehaviour":
        ...
        with open(r"C:\TMP\lyrics_fetch\storyrace_020290011_typetree.json", 'w', encoding='utf-8') as f:
            json.dump(tree, f, ensure_ascii=False, indent=2)
        print(f"Written to file. Keys: {list(tree.keys())}")
        td = tree.get('textData', [])
        print(f"textData count: {len(td)}")
        for item in td:
            has_text = 'text' in item
            print(f"  key={item.get('key')} cue={item.get('cueName')} has_text={has_text} text={repr(item.get('text', 'MISSING'))[:100]}")
