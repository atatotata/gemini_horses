...
def save_checkpoint():
    ...
    tmp_path = ROOT_DIR / "extra_checkpoint.json.tmp"
    try:
...
def main():
    print("=== Starting Extra Stories Translation Pipeline ===")
    load_checkpoint()
    ...
    if untranslated_names:
        print(f"\nTranslating {len(untranslated_names)} speaker names...")
        name_batch_size = 100
    ...
    print(f"\nDispatched {len(threads)} worker threads.")
    ...
    try:
        while True:
            ...
            print(f"[Status] Remaining -> Choices: {cq}, Dialogues: {dq} | Checkpoint: {len(checkpoint)}")
